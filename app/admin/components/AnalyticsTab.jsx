...
const AnalyticsTab = () => {
	...
	const filteredAnalytics = useMemo(() => {
		...
		return analytics.filter(record => {
			...
			const visitDate = record.firstVisit.toDate ? record.firstVisit.toDate() : new Date(record.firstVisit);
	...
	const chartData = useMemo(() => {
		...
		if (timeRange === "24h") {
			for (let i = 23; i >= 0; i--) {
				...
				days[hourKey] = { date: formatDate(d, true), visitors: 0, views: 0, sortKey: d.getTime() };
			}
			...
			for (let i = numDays - 1; i >= 0; i--) {
				...
				days[dateStr] = { date: formatDate(d), visitors: 0, views: 0, sortKey: d.getTime() };
			}
	...
	const mapCenter = useMemo(() => {
		...
		const avgLat = locationGroups.reduce((sum, group) => sum + group.latitude, 0) / locationGroups.length;
		const avgLng = locationGroups.reduce((sum, group) => sum + group.longitude, 0) / locationGroups.length;
		return [avgLat, avgLng];
	}, [locationGroups]);

	const mapZoom = useMemo(() => (locationGroups.length === 0 ? 2 : locationGroups.length === 1 ? 10 : 3), [locationGroups]);

	// Calculate totals
	const stats = useMemo(() => {
		const totalVisitors = filteredAnalytics.length;
		const totalViews = filteredAnalytics.reduce((acc, curr) => acc + (curr.visitCount || 1), 0);
		const singleVisitUsers = filteredAnalytics.filter(a => (a.visitCount || 1) === 1).length;
		const bounceRate = totalVisitors ? Math.round((singleVisitUsers / totalVisitors) * 100) : 0;

		return { visitors: totalVisitors, views: totalViews, bounceRate: bounceRate };
	}, [filteredAnalytics]);
