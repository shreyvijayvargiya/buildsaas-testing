	...
	const defaultRenderOption = (option, index) => {
		...
		return (
			<motion.button
				...
				className={`w-full px-4 py-1.5 text-left flex items-center gap-2 transition-colors text-sm ${value === option.value
			...
			<AnimatePresence>
				{isOpen && (
					<motion.div
						...
						className={`absolute top-full left-0 mt-1 w-full bg-white border border-zinc-200 rounded-xl shadow-lg z-[60] overflow-hidden ${dropdownClassName}`}
					>
